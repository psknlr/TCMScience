...
def retrieve_tcm_evidence(subject: str, *, claim_kind: str = "efficacy",
    ...
    # The claim this skill is entitled to make is about *the search*, not about
    # the subject: that the corpus records the subject. It must cite a record
    # that licenses that kind of claim — a classical text for an attribution,
    # else a classical or expert source for a traditional use. A search that
    # found only studies makes no coverage claim; its evidence table says the rest.
    cited, kind = None, ""
    for kind in ("attribution", "traditional_use"):
        fitting = [e for e in evidence if e.usable and licenses(e.tier, kind)]
        if fitting:
            cited = max(fitting, key=lambda e: (e.tier, e.id))
            break
    if cited is not None:
        ...
        claims.append(CandidateClaim(
            ...
            claim_kind=kind, subject=subject, predicate="recorded_in",
            object=SEED_SOURCE_ID, supports=(cited.id,),
