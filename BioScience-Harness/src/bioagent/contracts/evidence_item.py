...
#:
#: They sit at ``COMPUTATIONAL_PREDICTION``, the tier below every observation, so
#: a prediction is never ranked as a bench experiment. That tier licenses a
#: ``mechanism_hypothesis`` and nothing stronger (``tcm.model.CLAIM_SUPPORT``):
#: a network-pharmacology skill may propose a mechanism, and
#: :func:`check_claim` refuses it as an established one.
...
#: module makes explicit rather than silent. Every predictive design lands on
#: COMPUTATIONAL_PREDICTION, for the reason stated above.
EVIDENCE_TIER_FOR_DESIGN: Mapping[str, EvidenceTier] = {
    ...
    **{d: EvidenceTier.COMPUTATIONAL_PREDICTION for d in PREDICTIVE_DESIGNS},
}
