...
- **解读**：各阈值下通过的通路名称不同，但背后的蛋白始终是同一组细胞色素 P450：CYP1A2（205 个化合物中 136 个有活性）、CYP2C9（206 中 78）、CYP2D6、CYP2E1，数据来自 Tox21 等对所有化合物统一检测的面板。Reactome 把这些酶归到"EET/HETE 合成"等通路里，但数据真正说明的是：**方剂成分常常抑制 CYP1A2/2C9**。这是**中药—药物相互作用**的信号（与经 CYP1A2/2C9 代谢的药物合用时需要注意），**不是**方剂疗效机制的证据。发布出来的"机制假说"应按这个意思理解；
