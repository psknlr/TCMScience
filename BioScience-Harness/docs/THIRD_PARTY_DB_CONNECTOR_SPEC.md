...
- 疾病基因集已通过 Open Targets 关联，见下一节。

**Open Targets 疾病关联快照（已完成）**：
- `sources/fetch_opentargets.py` 与 `scripts/fetch_opentargets.py`：唯一联网的一步。通过 GraphQL 分页（每页 3000，接口上限）取回一个疾病的全部关联靶点，连同 API 版本、数据版本和取回时间存成一个原始 JSON；条数与接口报告的总数不符时报错，不生成缺数据的快照。请求走 `HTTPBackend`（限速、重试、`Retry-After`）；
- `parsers/opentargets.py`：疾病节点用 MONDO/EFO CURIE（附 DOID、ICD10 等交叉引用）；靶点优先用 Swiss-Prot 号（`uniprot:`），没有的保留 Ensembl 号并计数。**每种证据类型单独成边**（总分、遗传关联、文献、临床、表达、动物模型……），知识等级随类型而定：遗传关联为 `statistical_association`，文献为 `text_co_occurrence`；
- 新研究设计 `evidence_aggregate`：汇总了多条证据的分数。和 `chemical_analysis` 一样**不对应证据等级、本身不支持任何主张**，且只能用于 `associated_with`；
- 快照版本 = 平台版本 + 疾病 ID，例如 `opentargets@26.06+MONDO_0005148`；数据源卡片：CC0-1.0，引用 doi:10.1093/nar/gkae1128；
- Skill 新增 `disease` 步骤（`in_silico`）：实测靶点与疾病基因集的重叠，超几何检验（与通路富集同一背景）加同一套度匹配置换零分布（独立随机流，不改变通路结果）。**只作为统计量报告，不生成主张**；每条通路主张附上其中与疾病相关的靶点，`targets.tsv` 附疾病分数，另写 `disease.json`。

**真实数据结果（2 型糖尿病，MONDO_0005148，Open Targets 26.06）**：
- 9,907 个关联靶点，快照 9,836 个靶点、22,171 条边，通过质量检查（335 个无 Swiss-Prot 号，按 Ensembl 号保留）；其中有遗传关联证据的 3,340 个，分数 ≥ 0.5 的 721 个；
- 默认定义（遗传关联 ≥ 0.5）：237 个实测靶点中 9 个是疾病基因（PPARG、SLC22A3、BCL2、ALDH2、ATP2A3、SLC22A2、ESR1、ADRA2A、GBA2），**倍数 0.95，p = 0.61，置换 p = 0.71，即没有富集**；
- 敏感性分析：

  | 疾病基因集定义 | 疾病基因（背景内） | 重叠 | 倍数 | 超几何 p | 置换 p |
  |---|---|---|---|---|---|
  | 遗传关联 ≥ 0.5（默认） | 486 | 9 | 0.95 | 0.61 | 0.71 |
  | 遗传关联，不设阈值 | 2,001 | 45 | 1.15 | 0.17 | 0.22 |
  | 遗传关联 ≥ 0.8 | 49 | 2 | 2.09 | 0.25 | 0.33 |
  | 总分 ≥ 0.5 | 126 | 8 | 3.26 | 0.003 | 0.011 |
  | 文献共现 ≥ 0.5 | 557 | 58 | 5.34 | 7.7e-27 | 0.001 |

- **解读**：一旦用文献来定义"糖尿病基因"，就出现极强的"富集"；用人类遗传证据则完全没有。这正是循环论证：成分—靶点数据和文献共现来自同一批被研究得多的蛋白。许多网络药理学文章报告的"与疾病靶点高度重叠"，很可能大部分来自这种循环。所以默认只用遗传关联，并在 `limitations.md` 中写明；选用文献或总分时会自动追加"部分循环"的说明；
- 这个阴性结果不说明方剂对糖尿病无效，只说明"实测靶点集中于糖尿病遗传基因"这一点**不成立**，不能拿来支持机制叙述。葛根芩连汤的经典主治是以证候表述的（协热下利），与现代疾病并非一一对应；选哪种疾病是分析者的决定，并记录在快照 ID 中；
- 66 条通路假说不受影响，结果摘要两次运行完全一致。

**筛选组合偏倚的对照：以"实测过的蛋白"为背景（已完成，现为默认）**：
- 之前的富集以完整的人类 Reactome 注释为背景。问题是天然产物总被拿去测同几组靶点（碳酸酐酶、转运体、CYP450），某条通路显著可能只是因为它的蛋白**被测过**，而不是因为被**命中**；
- 新的默认背景是"实测过的蛋白"：这些成分测过的每个人类蛋白，无论有没有活性（活性类型为 IC50/Ki/Kd/EC50，任何数值，含删失值）。问题随之变成"在测过的蛋白里，强活性结果是否集中在某条通路"。通路的大小范围仍按 Reactome 原始定义，检验只计背景内的成员；背景内没有成员的通路不检验。`--background reactome` 保留原做法；
- **真实数据结果**：测过的人类蛋白 391 个，其中 237 个达到 ≤ 10 µM。以此为背景检验了 959 条通路，**没有一条显著**；原来 66 条显著的通路全部消失。例如"二氧化碳可逆水合"通路的 12 个蛋白全都测过，其中 11 个命中；"阿巴卡韦跨膜转运"也是 5 个测过、5 个命中。与 2 型糖尿病遗传基因的重叠同样没有富集（测过的蛋白里有 16 个是疾病基因，9 个命中，倍数 0.93）；
- **这个对照本身的局限**：测过的蛋白里 61% 被记为强活性。原因是数据库很少收录阴性结果，所以这个背景同样偏向命中，检验功效很低（任何通路的富集倍数最多约 1.65）。所以"这里不显著"不等于"与该通路无关"。命中率和可达到的最大倍数会自动写进 `limitations.md` 和 `provenance.json`；
- **结论**：就这批公开数据而言，"葛根芩连汤的成分作用于某些通路"的富集证据几乎完全可以用"哪些蛋白被测过"来解释。要得到可检验的机制假说，需要无偏的活性谱数据，例如对同一组成分做全面板筛选并同时报告阴性结果，而不是更多的数据库合并。在两种背景下，工具都如实报告了结果，但默认只在"实测过的蛋白"背景下生成主张，因此默认运行不产生任何主张。

**PubChem BioAssay：含阴性结果的筛选数据（已完成，作为 `--hits screening` 可选模式）**：
- 目的：上一节的对照检验功效不足，是因为整理型数据库几乎只收录阳性结果。PubChem BioAssay 保存了提交者的全部判定，包括阴性；
- `sources/fetch_pubchem.py` 与 `scripts/fetch_pubchem.py`：唯一联网的一步。先把 InChIKey 批量转成 CID，再按 CID 批量取回每个化合物的全部检测结果。批次失败（超时或过大）就二分重试直到单个 ID；二分也救不回来就报错，不会悄悄少掉化合物。PubChem 没有数据（404）记为空；
- `parsers/pubchem_bioassay.py`：判为有活性 → `targets`；判为无活性 → 新谓词 **`tested_against`**，也就是"测过、无活性"这种阴性结果。结论不确定或未指明的结果、没有蛋白靶点的、RNAi 筛选、映射不到唯一人类 UniProt 号的，都丢弃并计数。引用就是检测本身（`pubchem.bioassay:<AID>`，任何人都能查），有 PMID 的一并附上。基因号 → UniProt 用 STRING 别名文件离线映射，与 STRING、Reactome 快照用同一套 ID；一个基因对应两个蛋白时视为有歧义，不映射；
- 数据源卡片：许可证写作"NCBI 数据政策"（NCBI 本身不加限制，但提交者可能保留权利，所以商业使用标为"未知"）；引用 doi:10.1093/nar/gkae1059。快照版本 = 取回日期 + 所查化合物集合的哈希；
- **真实数据**：1,792 个成分中 1,768 个在 PubChem 有 CID，共 422,390 条检测结果；保留 143,702 条落在人类蛋白上的明确判定（10,710 条有活性，132,992 条无活性），通过质量检查；
- **为什么不能按蛋白计"命中"**：有 1,768 个化合物，几乎每个测过的蛋白都会有某个化合物有活性。测过 20 个以上化合物的 116 个蛋白中，80% 至少有一次命中，按蛋白计已经饱和。所以检验改为以"化合物—蛋白检测"为单位：某通路的蛋白上，活性检测占多少比例。零分布整组抽取蛋白（连同它们的全部检测），用来处理同一蛋白的检测彼此不独立的问题。整体活性比例为 **8.6%**（12,579 次检测中 1,080 次有活性），不是整理型数据中的 61%，所以这个背景有真正的检验功效；
- **结果（10,000 次置换）**：

  | 每个蛋白至少测过的化合物数 | 参与的蛋白 | 检验的通路 | 通过 BH（q ≤ 0.05）的通路 |
  |---|---|---|---|
  | 10 | 130 | 68 | 二氧化碳可逆水合（碳酸酐酶，q = 0.014） |
  | 20（默认） | 116 | 58 | 16-20-HETE 合成（q = 0.023）、EET/DHET 合成（q = 0.026） |
  | 50 | 98 | 49 | 16-20-HETE 合成（q = 0.017）、类 maresin SPM 合成（q = 0.017） |

- **解读**：默认阈值（20）和阈值 50 下通过的通路名称不同，但背后的蛋白是同一组细胞色素 P450：CYP1A2（205 个化合物中 136 个有活性）、CYP2C9（206 中 78）、CYP2D6、CYP2E1，数据来自 Tox21 等对所有化合物统一检测的面板。Reactome 把这些酶归到"EET/HETE 合成"等通路里，但数据真正说明的是：**方剂成分常常抑制 CYP1A2/2C9**。这是**中药—药物相互作用**的信号（与经 CYP1A2/2C9 代谢的药物合用时需要注意），**不是**方剂疗效机制的证据。发布出来的"机制假说"应按这个意思理解；
- 阈值为 10 时出现的碳酸酐酶，来自小规模、按阳性挑选的文献检测（例如 CA1 测了 21 个化合物、19 个有活性，CA7 测了 26 个、24 个有活性），阈值提高后就消失了；CYP1B1 的 22/22 也属于这种情况。筛选模式的 `limitations.md` 写明了这一点，也写明了光学读数干扰（如小檗碱的荧光）等假阳性来源；
- 与 2 型糖尿病遗传基因：只有 6–8 个疾病蛋白被充分测过，活性比例倍数 1.2–1.4，p = 0.24–0.33，**仍无富集**；
- 两次运行结果摘要一致。默认运行（整理型数据 + 实测背景）不受影响，仍不产生主张，PubChem 结果被排除并计数。
