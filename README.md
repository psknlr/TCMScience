...
| **2** | 首个让**计算预测在类型层面无法冒充临床事实**的系统 | 网络药理学输出的是预测关系，但文献里它常被表述成「作用机制」。我们把它做成内核的**类型约束**：预测性证据在设计表中只出现在 `MECHANISM_HYPOTHESIS` 一行，永远无法支撑 `MECHANISTIC`/`CLINICAL`/`ASSOCIATION`/`SAFETY` |
...
_SUPPORTS = {
    ...
    ClaimType.MECHANISM_HYPOTHESIS: {"in_vitro", "animal"} | PREDICTIVE_DESIGNS,  # ← 唯一一行
    ClaimType.MECHANISTIC:  {"in_vitro", "animal"},                    # ← 预测够不着
